...
# require 'pry'
#require 'pry-byebug'
@sin_present = /\b\d{3}-\d{3}-\d{3}\b/
@any_sin = /\b(\d{3})[-.]?(\d{3})[-.]?(\d{3})\b/
...
def grab_sin(string)
  @sin_present.match(string) { |match| match.to_s}
end
...
def hide_all_sins(string)
  if has_sin?(string)
   string_new = string.gsub(@any_sin, "XXX-XXX-" + '\3') 
  else
    string
  end
end
...
def format_sins(string) 
  new_string = string.gsub(@any_sin, '\1-' + '\2-' + '\3' )
